...
impl TlsManager {
    ...
    pub fn generate_self_signed_cert(
        ...
        let _pkcs8_bytes =
            EcdsaKeyPair::generate_pkcs8(alg, &rng).map_err(|_| "Failed to generate key pair")?;
...
fn load_certs(path: &str) -> Result<Vec<Certificate>, Box<dyn std::error::Error + Send + Sync>> {
    let cert_data =
        fs::read(path).map_err(|e| format!("Failed to read certificate file {}: {}", path, e))?;
...
fn load_private_key(path: &str) -> Result<PrivateKey, Box<dyn std::error::Error + Send + Sync>> {
    let key_data =
        fs::read(path).map_err(|e| format!("Failed to read private key file {}: {}", path, e))?;
...
pub mod middleware {
    ...
    /// HTTPS重定向中间件
    pub async fn https_redirect_middleware(request: Request, next: Next) -> Response {
        // 检查是否是HTTP请求
        let host = request
            .headers()
            .get(header::HOST)
