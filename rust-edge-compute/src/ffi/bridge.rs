...
// 重新导出核心类型
use rust_edge_compute_core::core::{ComputeRequest, ComputeResponse};
...
impl CppAlgorithmExecutor {
    ...
    fn create_algorithm_input(&self, request: &ComputeRequest) -> Result<ffi::AlgorithmInput> {
        let parameters_json = request.parameters.clone();

        // 从参数中提取设备ID，如果没有则使用默认值
        let device_id = if let Ok(params) = serde_json::from_str::<serde_json::Value>(&parameters_json) {
