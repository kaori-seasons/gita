...
impl ImagePreprocessor {
    ...
    fn image_to_tensor(image: &[u8], device: &Device) -> Result<Tensor> {
        ...
        // 创建Tensor [C, H, W]
        let tensor = Tensor::from_slice(&data, (channels, height, width), device)?;

        Ok(tensor)
    }

    /// 归一化图像
    fn normalize_image(tensor: Tensor, mean: &[f32; 3], std: &[f32; 3]) -> Result<Tensor> {
        // 应用归一化：(x - mean) / std
        ...
        // 不应该对结果使用 `?`，因为不是 Result类型
        let normalized = (&tensor - &mean_tensor)
            .ok()
            .and_then(|r| (&r / &std_tensor).ok())
            .ok_or_else(|| EdgeComputeError("Tensor normalization failed".to_string()))?;
        Ok(normalized)
    }
}
...
impl AudioPreprocessor {
    ...
    fn to_mel_spectrogram(
        ...
        let mel = vec![vec![0.0f32; n_mels]; n_frames];
    ...
    fn audio_to_tensor(samples: &[f32], device: &Device) -> Result<Tensor> {
        Tensor::from_slice(samples, (1, samples.len()), device)
            .map_err(|e| EdgeComputeError(format!("Failed to create audio tensor: {}", e)))
    }
    ...
    fn mel_to_tensor(mel: &[Vec<f32>], device: &Device) -> Result<Tensor> {
        ...
        Tensor::from_slice(&data, (1, n_frames, n_mels), device)
            .map_err(|e| EdgeComputeError(format!("Failed to create mel tensor: {}", e)))
    }
}
...
impl TextPreprocessor {
    /// 预处理文本（tokenization等）
    pub fn preprocess(text: &str, max_length: Option<usize>, device: &Device) -> Result<Tensor> {
        ...
        Tensor::from_slice(&padded, (1, max_len), device)
            .map_err(|e| EdgeComputeError(format!("Failed to create text tensor: {}", e)))
    }
}
