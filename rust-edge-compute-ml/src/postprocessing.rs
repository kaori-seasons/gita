...
impl Postprocessor {
    ...
    pub fn extract_classifications(
        ...
        // 按分数排序
        class_scores.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(Ordering::Equal));
    ...
    pub fn softmax(tensor: &Tensor) -> Result<Tensor> {
        ...
        exp.broadcast_div(&sum)
            .map_err(|e| EdgeComputeError(format!("Failed to apply softmax: {}", e)))
    }

    /// 从文本生成结果解码
    pub fn decode_text_generation(token_ids: &Tensor, vocab: &[String]) -> Result<String> {
        // 获取token IDs
