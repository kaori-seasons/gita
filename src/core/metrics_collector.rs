...
pub mod system_metrics {
    ...
    pub fn get_mapped_bytes() -> u64 {
        if let Ok(content) = fs::read_to_string("/proc/self/maps") {
            content
                .lines()
                .filter(|line| line.contains("[heap]") || line.contains("[stack]") || line.contains("r--p"))
                .count() as u64 * 4096  // 估算每个映射段约 4KB
        } else {
            0
        }
    }
...
impl CoreMetrics {
    ...
    pub fn increment_completed_tasks(&self) {
        self.scheduler_tasks_completed_total.fetch_add(1, Ordering::Relaxed);
    }

    pub fn increment_timeouts(&self) {
        self.scheduler_task_timeouts_total.fetch_add(1, Ordering::Relaxed);
    }

    pub fn increment_retries(&self) {
        self.scheduler_task_retries_total.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub async fn record_ffi_call_duration(&self, duration_ms: f64) {
        ...
        // 保持最近1小时的样本
        if durations.len() > 3600 {
            durations.drain(0..(durations.len() - 3600));
        }
    }
    ...
    pub fn increment_active_containers(&self) {
        self.container_instances_active.fetch_add(1, Ordering::Relaxed);
    }

    pub fn decrement_active_containers(&self) {
        self.container_instances_active.fetch_sub(1, Ordering::Relaxed);
    }
    ...
    pub fn increment_created_containers(&self) {
        self.container_instances_created_total.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub async fn start_system_metrics_monitor(&self) {
        let metrics = Arc::clone(&Arc::new(self.clone()));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // 更新内存指标
                let rss = system_metrics::get_rss_bytes();
                metrics.set_rss_bytes(rss).await;
                
                let vm_total = system_metrics::get_vm_total_bytes();
                metrics.set_vm_total_bytes(vm_total).await;
                
                // 计算内存使用率（简化：RSS / VM总大小）
                if vm_total > 0 {
                    let mem_percent = (rss as f64 / vm_total as f64) * 100.0;
                    metrics.set_memory_usage_percent(mem_percent).await;
                }
                
                // 更新总内存字节数
                metrics.set_memory_bytes(rss).await;
            }
        });
    }
    ...
    pub fn export_prometheus_metrics(&self) -> String {
        ...
        output.push_str(&format!("container_instances_active {}\n", self.get_active_containers()));
        ...
        output.push_str(&format!("container_instances_created_total {}\n", self.get_created_containers()));

        // 错误率
        output.push_str("# HELP performance_error_rate_percent 错误率百分比\n");
        output.push_str("# TYPE performance_error_rate_percent gauge\n");
        output.push_str(&format!("performance_error_rate_percent {:.2}\n", self.get_error_rate()));
...
/// 导出 Prometheus 格式的指标
pub fn export_prometheus_metrics(metrics: &CoreMetrics) -> String {
    let mut output = String::new();

    // 任务调度指标
    output.push_str(&format!("# HELP {} 当前正在执行的任务数\n", metric_names::SCHEDULER_ACTIVE_TASKS));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::SCHEDULER_ACTIVE_TASKS));
    output.push_str(&format!("{} {}\n", metric_names::SCHEDULER_ACTIVE_TASKS, metrics.get_active_tasks()));

    output.push_str(&format!("# HELP {} 等待执行的任务数\n", metric_names::SCHEDULER_QUEUED_TASKS));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::SCHEDULER_QUEUED_TASKS));
    output.push_str(&format!("{} {}\n", metric_names::SCHEDULER_QUEUED_TASKS, metrics.get_queued_tasks()));

    output.push_str(&format!("# HELP {} 已完成任务总数\n", metric_names::SCHEDULER_TASKS_COMPLETED_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::SCHEDULER_TASKS_COMPLETED_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::SCHEDULER_TASKS_COMPLETED_TOTAL, metrics.get_completed_tasks()));

    output.push_str(&format!("# HELP {} 任务超时总数\n", metric_names::SCHEDULER_TASK_TIMEOUTS_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::SCHEDULER_TASK_TIMEOUTS_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::SCHEDULER_TASK_TIMEOUTS_TOTAL, metrics.get_timeouts()));

    // HTTP 请求指标
    output.push_str(&format!("# HELP {} HTTP请求总数\n", metric_names::HTTP_REQUESTS_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::HTTP_REQUESTS_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::HTTP_REQUESTS_TOTAL, metrics.get_http_requests()));

    output.push_str(&format!("# HELP {} HTTP错误总数\n", metric_names::HTTP_ERRORS_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::HTTP_ERRORS_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::HTTP_ERRORS_TOTAL, metrics.get_http_errors()));

    output.push_str(&format!("# HELP {} 活跃HTTP连接数\n", metric_names::HTTP_CONNECTIONS_ACTIVE));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::HTTP_CONNECTIONS_ACTIVE));
    output.push_str(&format!("{} {}\n", metric_names::HTTP_CONNECTIONS_ACTIVE, metrics.get_active_connections()));

    // 系统资源指标
    output.push_str(&format!("# HELP {} CPU使用率百分比\n", metric_names::SYSTEM_CPU_USAGE_PERCENT));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::SYSTEM_CPU_USAGE_PERCENT));
    output.push_str(&format!("{} {:.2}\n", metric_names::SYSTEM_CPU_USAGE_PERCENT, metrics.get_cpu_usage().await));

    output.push_str(&format!("# HELP {} 内存使用字节数\n", metric_names::SYSTEM_MEMORY_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::SYSTEM_MEMORY_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::SYSTEM_MEMORY_BYTES, metrics.get_memory_bytes().await));

    output.push_str(&format!("# HELP {} 内存使用率百分比\n", metric_names::SYSTEM_MEMORY_USAGE_PERCENT));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::SYSTEM_MEMORY_USAGE_PERCENT));
    output.push_str(&format!("{} {:.2}\n", metric_names::SYSTEM_MEMORY_USAGE_PERCENT, metrics.get_memory_usage_percent().await));

    // 内存分类指标
    output.push_str(&format!("# HELP {} Rust堆内存字节数\n", metric_names::MEMORY_RUST_HEAP_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_RUST_HEAP_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_RUST_HEAP_BYTES, metrics.get_rust_heap_bytes()));

    output.push_str(&format!("# HELP {} C++堆内存字节数\n", metric_names::MEMORY_CPP_HEAP_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_CPP_HEAP_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_CPP_HEAP_BYTES, metrics.get_cpp_heap_bytes()));

    output.push_str(&format!("# HELP {} 共享内存字节数\n", metric_names::MEMORY_SHARED_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_SHARED_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_SHARED_BYTES, metrics.get_shared_memory_bytes()));

    // 虚拟内存、RSS 和映射内存指标
    output.push_str(&format!("# HELP {} 虚拟内存总字节数\n", metric_names::MEMORY_VM_TOTAL_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_VM_TOTAL_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_VM_TOTAL_BYTES, metrics.get_vm_total_bytes().await));

    output.push_str(&format!("# HELP {} 常驻内存字节数\n", metric_names::MEMORY_RSS_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_RSS_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_RSS_BYTES, metrics.get_rss_bytes().await));

    output.push_str(&format!("# HELP {} 内存映射字节数\n", metric_names::MEMORY_MAPPED_BYTES));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::MEMORY_MAPPED_BYTES));
    output.push_str(&format!("{} {}\n", metric_names::MEMORY_MAPPED_BYTES, metrics.get_mapped_bytes().await));
    ...
    output.push_str(&format!("# HELP {} 平均响应时间毫秒\n", metric_names::PERFORMANCE_AVG_RESPONSE_TIME_MS));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::PERFORMANCE_AVG_RESPONSE_TIME_MS));
    output.push_str(&format!("{} {:.2}\n", metric_names::PERFORMANCE_AVG_RESPONSE_TIME_MS, avg_response_time));

    output.push_str(&format!("# HELP {} P95响应时间毫秒\n", metric_names::PERFORMANCE_P95_RESPONSE_TIME_MS));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::PERFORMANCE_P95_RESPONSE_TIME_MS));
    output.push_str(&format!("{} {:.2}\n", metric_names::PERFORMANCE_P95_RESPONSE_TIME_MS, p95_response_time));

    output.push_str(&format!("# HELP {} P99响应时间毫秒\n", metric_names::PERFORMANCE_P99_RESPONSE_TIME_MS));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::PERFORMANCE_P99_RESPONSE_TIME_MS));
    output.push_str(&format!("{} {:.2}\n", metric_names::PERFORMANCE_P99_RESPONSE_TIME_MS, p99_response_time));

    output.push_str(&format!("# HELP {} 错误率百分比\n", metric_names::PERFORMANCE_ERROR_RATE_PERCENT));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::PERFORMANCE_ERROR_RATE_PERCENT));
    output.push_str(&format!("{} {:.2}\n", metric_names::PERFORMANCE_ERROR_RATE_PERCENT, error_rate));

    // FFI 指标
    output.push_str(&format!("# HELP {} FFI调用总数\n", metric_names::FFI_CALLS_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::FFI_CALLS_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::FFI_CALLS_TOTAL, metrics.get_ffi_calls()));

    output.push_str(&format!("# HELP {} FFI错误总数\n", metric_names::FFI_ERRORS_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::FFI_ERRORS_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::FFI_ERRORS_TOTAL, metrics.get_ffi_errors()));

    // 容器指标
    output.push_str(&format!("# HELP {} 活跃容器数\n", metric_names::CONTAINER_INSTANCES_ACTIVE));
    output.push_str(&format!("# TYPE {} gauge\n", metric_names::CONTAINER_INSTANCES_ACTIVE));
    output.push_str(&format!("{} {}\n", metric_names::CONTAINER_INSTANCES_ACTIVE, metrics.get_active_containers()));

    output.push_str(&format!("# HELP {} 创建的容器总数\n", metric_names::CONTAINER_INSTANCES_CREATED_TOTAL));
    output.push_str(&format!("# TYPE {} counter\n", metric_names::CONTAINER_INSTANCES_CREATED_TOTAL));
    output.push_str(&format!("{} {}\n", metric_names::CONTAINER_INSTANCES_CREATED_TOTAL, metrics.container_instances_created_total.load(Ordering::Relaxed)));
