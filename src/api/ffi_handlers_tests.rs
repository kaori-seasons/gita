...
mod tests {
    ...
    async fn test_cpp_algorithm_request_serialization() {
        ...
        assert_eq!(request.parameters.get("threshold").and_then(|v| v.as_f64()), Some(0.5));
    }
    ...
    async fn test_cpp_algorithm_request_with_complex_parameters() {
        ...
        // 验证嵌套结构
        assert_eq!(request.parameters.get("threshold").and_then(|v| v.as_f64()), Some(0.5));
        assert_eq!(request.parameters.get("window_size").and_then(|v| v.as_i64()), Some(128));
    ...
    async fn test_response_serialization() {
        ...
        assert_eq!(response.get("algorithm").and_then(|v| v.as_str()), Some("vibrate31"));
        assert_eq!(
    ...
    async fn test_error_response_serialization() {
        ...
        // 验证错误响应结构
        assert!(!error_response.get("success").and_then(|v| v.as_bool()).unwrap_or(true));
        assert_eq!(
