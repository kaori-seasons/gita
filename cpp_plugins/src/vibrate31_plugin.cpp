...
std::string Vibrate31Plugin::getDescription() const {
    return "vibration feature extraction plugin v31 based on spectrum analysis";
}
...
bool Vibrate31Plugin::validateParameters() {
    if (sampling_rate_ <= 0) {
        setError("sampling_rate must be greater than 0");
        return false;
    }
    
    if (duration_limit_ <= 0) {
        setError("duration_limit must be greater than 0");
        return false;
    }
...
bool Vibrate31Plugin::computeVibrationFeatures(const std::vector<double>& wave_data,
    ...
    try {
        ...
        if (duration < duration_limit_) {
            setError("wave duration insufficient for spectrum analysis");
            return false;
        }
        ...
        if (dc_threshold_ > 0) {
            ...
            if (dc_value >= dc_threshold_) {
                setError("serious dc interference in waveform: " + std::to_string(dc_value));
                return false;
            }
        }
        ...
        if (!segmentByStatus(wave_data, speed_data, segments, statuses)) {
            setError("segmentation by status failed");
            return false;
        }
        
        if (segments.empty()) {
            setError("no valid operating condition batches detected");
            return false;
        }
        ...
        setError("exception in vibration feature calculation: " + std::string(e.what()));
        return false;
    }
}
...
bool Vibrate31Plugin::computeSegmentFeatures(const std::vector<double>& segment_wave,
    ...
    try {
        ...
        setError("exception in segment feature calculation: " + std::string(e.what()));
        return false;
    }
}
...
bool Vibrate31Plugin::mergeSegmentFeatures(const std::vector<std::map<std::string, double>>& segment_features,
    ...
    try {
        ...
        setError("exception in feature merge: " + std::string(e.what()));
        return false;
    }
}
...
bool Vibrate31Plugin::computeSpectrum(const std::vector<double>& wave_data,
    ...
    try {
        ...
        setError("spectrum calculation exception: " + std::string(e.what()));
        return false;
    }
}
...
bool Vibrate31Plugin::segmentByStatus(const std::vector<double>& wave_data,
    ...
    try {
        ...
        if (wave_data.size() < (size_t)(sampling_rate_ * duration_limit_)) {
            segments.push_back(wave_data);
            statuses.push_back(1);
            return true;
        }
        
        int segment_size = sampling_rate_ * 30;
        ...
        setError("segmentation exception: " + std::string(e.what()));
        return false;
    }
}
